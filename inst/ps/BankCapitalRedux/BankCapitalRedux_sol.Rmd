...
library(RTutor)
#library(restorepoint)  
setwd("~/Desktop/Masterarbeit/Problemset")

ps.name = "BankCapitalRedux"; sol.file = paste0(ps.name,"_sol.Rmd")

libs = c("tidyverse", "lfe", "gridExtra", "mfx", "texreg", "stargazer", "glueformula", "ggthemes", "gplots", "stats", "pROC") # character vector of all packages you load in the problem set

name.rmd.chunks(sol.file) # set auto chunk names in this file

create.ps(sol.file=sol.file, ps.name=ps.name, user.name=NULL,libs=libs, stop.when.finished=FALSE, addons="quiz")

show.ps(ps.name, load.sav=FALSE,  sample.solution=FALSE, is.solved=FALSE, catch.errors=TRUE, launch.browser=TRUE)
...
**Importing RDS files into R**

If you have created a data set in an R project and want to use it later in another project, the easiest way is to save your data set as an `.rds` file. Then, you can load this with the `readRDS()` function.

```{r "2_1",optional=TRUE, eval=FALSE}
# Import 
...
An extremely important data structure in R is the so-called **data frame**. Technically, a data frame is a list whose elements are vectors with the same size. The vectors can be *numeric*, *logical*, *character* or *factor* variables. 

In the following I will introduce you great functions for dealing with data frames:

- Create a data frame:

```{r "2_3",optional=TRUE, eval=FALSE}
df <- data.frame(
  ID = c(1, 2, 3, 4)
  age = c(20, 40, 60, 30),
  eyecolor = c("brown", "blue", "blue", "brown")
)
```

- Get column names of data frame:

```{r "2_4",optional=TRUE, eval=FALSE}
colnames(df)
```

- Access to a column with `$` operator:

```{r "2_5",optional=TRUE, eval=FALSE}
df$age
```

- Create new column:

```{r "2_6",optional=TRUE, eval=FALSE}
df$height <- c(1.6, 1.75 , 1.7, 1.68) 
```
...
question: How many variables does the data set contain?
answer: 169
#>

#< quiz "Number of rows"
question: How many rows does the data set contain?
answer: 2482
#>
...
#> 
unique(bankdata$country)

#< hint
display("You only need to put bankdata$country in between the brackets of the function unique().")
#>
```
...
In the following, we will look at the most common data types used in data analysis *(Wooldridge, 2018)*:

**Cross-sectional data**: This type of data consists of a single time period and many subjects such as individuals, age or countries. The ordering of the data doesn't matter. 

**Time Series Data**: This type of data shows us observations collected over time intervals which allows us to track changes over time. We have no randomly recorded data points, i.e. the data is typically presented in a chronological order.

**Panel data** is a cross-sectional time-series data in which we have data from the same set of entities at multiple points in time. If we have for each individual the same number of time points, then we have a balanced panel.
#>

Now let's do our first quiz. 

#< quiz "Data type"
question: What do you think? What type of data set is *bankdata*? 
sc:
...
#< quiz "Long format vs wide format"
question: Is bankdata a data in wide or long format? 
sc:
- Wide format
- Long format*
success: Great, your answer is correct!
...
#< award "Starter"
Great! First of all, congratulations on winning your first award! In this exercise, you have learned some of the basic R-functions. Let's win many more awards in the next tasks.
#>
...
The committee's first accord, the so-called Basel I accord, was introduced in 1988. The Basel Committee imposed global minimum capital requirements on international banks to improve the *stability of the financial sector*. The fundamental reason of Basel I was the fear that losses could not be absorbed because banks' capital adequacy would be too low in the event of unexpected loan defaults.

<img src="Data/Basel1.png" style="width: 50%; height: 50%">

*Figure 3: "Basel 1: Components of Capital"; Source: own illustration*

The committee divided the equity into two parts: 

- Tier 1 capital (the core capital) to preserve solvency:
  - Paid-up capital
  - Disclosed reserves

- Tier 2 capital (supplementary capital) to protect depositors from bank failure:
  - Revaluation Reserves
  - Undisclosed reserves
  - Hybrid capital instruments

Two capital requirements have been defined:

First Requirement: $$\frac{ \text{Tier 1 Capital}}{ \text{Risk weighted assets (RWA)}}> 4 \% $$

Second Requirement: $$\frac{ \text{Tier 1 Capital + Tier 2 Capital}}{ \text{Risk weighted assets (RWA)}}> 8 \% $$

The focus of this accord was entirely on credit risk. Therefore, each bank's asset has been allocated to a credit risk level (0%, 20%, 50%, 100%).

Due to an increase in trading activities of banks, in 1996, market price risks were also included in the capital requirement. In 1999, new negotiations were held, since the goals of Basel I were not achieved, as the calculation method represented the credit risk superficially *(Moosa, 2013)*.

#>

#< info "Basel II"

In 2004, after several negotiations, the Basel II reform package was finally adopted. The main objective of this regulation is to align regulatory capital requirements more closely with actual risk. Basel II includes three different risks: the *credit risk, the market risk and the operational risk*. Moreover, banks need to assess and collateralise the loans on the basis of ratings. 

Moreover, the Basel II concept introduced three pillars:

<img src="Data/Basel2_pillars.png" style="width: 45%; height: 45%">

*Figure 4: "Three pillars of Basel 2"; Source: own illustration*

The *minimum capital requirement* of 8% (Tier 1 capital 4% and Tier 2 capital 4%) has not been changed, but a part of the Tier 1 capital, the so-called *common Tier 1 common capital ratio* (CET1) should comprise at least 2% of the risk weighted assets, which consists of e.g. own shares and retained earnings.

For more information, click [here](https://www.bis.org/publ/bcbs107.pdf).
#>
...
**LCR**: The **Liquidity Coverage Ratio** (LCR) is intended to demonstrate the bank’s solvency under a severe 30-day stress scenario. It is calculated by dividing highly liquid assets, that means assets that can be converted into cash in a short time of period, by the expected 30-days net cash flows. If the LCR does not fall below 100%, the liquidity of the institution in the short term is according to banking supervisors sufficient. $$LCR = \frac{High \text{-} quality \ liquid \ assets \ (HQLA)}{total \ net \ cash \ flows \ over \ a \ 30\text{-}day \ stress \ period} $$

The rate was gradually increased from 60 percent in 2015 to 70 percent in 2016 and from 80 percent in 2017 to 100 percent in 2018. Moreover, banks are required to report the liquidity coverage ratio on a monthly basis.

**NSFR**: The **Net Stable Funding Ratio** (NSFR) was introduced as a further important liquidity measure. It is calculated by dividing the Available Stable Funding (ASF) by the Required Stable Funding (RSF) over a one-year period. $$NSFR = \frac{Available \ Stable \ Funding \ (ASF)}{Required \ Stable \ Funding \ (RSF)}$$
The goal of the NSFR ratio is that for the observation period of at least one year, the bank's illiquid assets are refinanced by stable sources. Thus, it is intended to limit excessive maturity transformation, i.e. to limit high refinancing of long-term loans by collecting short-term customer deposits, which was also a cause of the Global Financial Crisis.

Click [here](https://www.bis.org/publ/bcbs107.pdf) to learn more about the liquidity requirements.
#>

Let's do a small quiz to check whether you have understood the Basel III accord.
#< quiz "Basel III"
question: Which ratio was not introduced by Basel III? 
sc:
...
#< info "Basel IV"
On 7th December 2017, the new **Basel IV** accord was presented, but they are not a new set of regulations and are therefore called as ["Basel III: Finalising post-crisis reforms"](https://www.bis.org/bcbs/publ/d424.pdf). The goal is, as in the previous accords, the stabilisation of the financial system and introducing common regulations for the banking sector on a global level. 

The most important agreement is the new definition of *output floor* at 72.5%. The output floor provides that the capital requirement that is calculated by internal models must be at least 72.5% of the capital requirements calculated by standardised approach for credit risks. This restricts the possibilities of reducing their credit risks in internal models. The level is to be reached gradually by 2027. 

<img src="Data/Output floor.png" style="width: 50%; height: 50%">

*Figure 6: "Output floor: Date and Output floor calibration"; Source: BIS, 2017*

In addition, several other measures have been adapted:

- introduction of a newly designed standardised approach for operational risks

- Revision of leverage ratio and leverage ratio buffer for global systemically important banks (G-SIBs)

- Revision of the standard approach for measuring credit risk aiming the risk sensitivity and robustness of the existing approach

For more information, I recommend the informative document of the Basel Committee on Banking Supervision. Therefore click [here](https://www.bis.org/bcbs/publ/d424.pdf).
#>
...
Logical operators compare two variables of the same object class with each other. The most important logical operators in R are: 

```{r "4_2",optional=TRUE, eval=FALSE}
x == y # exactly equal
x != y # not equal
x < y # less than
x > y # more than
x <= y # less than or equal to
x >= y # more than or equal to
x | y # x OR y
x & y # x AND y
x %in% y # check if a value x is contained in a vector y
```
...
#< award "Dplyr Level 1"
Awesome! You have learned one of the most essential functions of data manipulation. Congratulations!
#>
...
```{r "4_5"}
#< hint
cat("Use the following expression to filter by Germany: country == 'Germany'")
#>
#< task
# Load dplyr package
library(dplyr)
...
The `labs()` function from the `ggplot2` package helps us to change the labelling of the axes, the title of the plot and the legend. It is structured as follows:

```{r "4_6",optional=TRUE, eval=FALSE}
labs(
  x = "",
  y = "",
  title = "", 
...
The `colour` argument allows us to change the name of the legend.
#>

#< info "Scaling"

We can scale the continuous data using the functions `scale_x_continuous()` or `scale_y_continuous()`.

```{r "4_7",optional=TRUE, eval=FALSE}
scale_x_continuous(
  Fractions = _,
  limits = _,
)
...
breaks: a numeric vector of the positions for breaks on the x- or y-axis

limits: a numeric vector indicating the minimum and maximum value

To learn more about the above functions, click [here](https://www.rdocumentation.org/packages/ggplot2/versions/1.0.0/topics/scale_x_continuous).
...
```{r "4_8"}
#< hint
cat("Since we want to display capital ratios in percentage, you need to multiplicate lev with 100")
#>
#< fill_in
...
# Line graph of the evolution of the capital ratio in %
ggplot(data = dat_germany, mapping = aes(x = year, y = lev*100)) +
  geom_line()  +
  scale_x_continuous(limits = c(1870, 2015), breaks = seq(1870, 2015, 20)) +
  labs(x = "year", y = "Capital ratio (%)", title = "Capital ratio Germany, full sample") +
  theme_bw()
...
# Capital ratio series by country
ggplot(data = bankdata, mapping = aes(x = year, y = lev*100)) +
  geom_line() +
  facet_wrap(~ country) +
  scale_x_continuous(limits = c(1870, 2015), breaks = seq(1870, 2015, 40)) +
  labs(x = "year", y = "Capital ratio (%)", title = "Capital ratio by country, full sample") +
...
The most well-known statistical measures are the empirical mean and the quantiles.  

**Mean**: The most frequently calculated mean is the arithmetic mean $\bar{x}$ for the observations $x_1, x_2, ..., x_n$. It is defined by the following formula $$\bar{x} = \frac{1}{n} \sum_{i=1}^{n} x_i$$

**Quantile**: For empirical quantiles, it is assumed that the given values $x_1^* ,x_2^* ,. . . ,x_n^*$ are ordered according to size. Using the p-Quantile, you want to determine the value for which $100⋅p$ percent of the data are to the left or $100⋅(1−p)$ percent of the data are to the right of this value.
$$\tilde{x}_{p} = \frac{1}{2} (x^*_{np} + x^*_{np+1}) \hspace{0.2 cm} , \ if \ np \ even$$
$$\tilde{x}_{p} = x^*_{\lceil np \rceil} \hspace{0.2 cm} , \ if \ np \ odd$$

**Median**: The empirical median  is called as the 0.5-quantile, a special case of the quantile. To be able to determine the empirical median, you need the ranking $x_1^* ,x_2^* ,. . . ,x_n^*$ of the observed values. The median is the observation value in the middle of this ordered observation series. It is defined by the following formula 
$$x_{Med} = \frac{x^*_{\frac{n}{2}} + x^*_{\frac{n}{2} +1}}{2} \hspace{0.2 cm} , \ if \ n \ even$$
$$x_{Med} = x^*_{\frac{n+1}{2}} \hspace{0.2 cm} , \ if \ n \ odd $$
...
# Summarise the data by year and calculate the statistical measures
summarise_data <- bankdata %>%
  group_by(year) %>%
  summarise(mean_lev = mean(lev, na.rm = TRUE)*100, 
            median_lev = median(lev, na.rm = TRUE)*100, 
            upper_lev = quantile(lev, probs = 0.75,  na.rm = TRUE)*100, 
            lower_lev = quantile(lev, probs = 0.25,  na.rm = TRUE)*100
            )

# Show the first five lines of summarise_data
head(summarise_data, 5)

```
#< award "Dplyr Level 2"
Awesome! You have learned how to use the `group_by()` and `summarise()` functions. Congratulations!
#>
...
```{r "4_13",fig.width=9}
#< hint
cat("Take the above output of summarise_data as a reference.")
#>
#< fill_in
...
# Plot of aggregated capital ratio from 1870 to 2015
ggplot(data = summarise_data) +
  geom_line(aes(x = year, y = mean_lev, color = "Mean")) +
  geom_line(aes(x = year, y = median_lev, color = "Median")) +
  geom_ribbon(aes(x = year, ymin = lower_lev, ymax = upper_lev, fill = "25th/75th percentile range"), alpha = 0.2) +
  labs(x = "year", y = "Capital ratio (%)", title = "Capital ratio, averages by year for 17 countries, full sample") +
  scale_x_continuous(breaks = seq(1870, 2015, 20)) +
  theme(legend.position = "right", legend.title = element_blank()) +
  theme_bw()
...
#< award "ggplot2 Level 1"
Very good! This is already a very important achievement since data visualization is very essential for data analysis. Keep it up!
#>
...
#< quiz "Decreasing capital ratio"
question: What could be a reason for the decreasing evolution of the capital ratio since 1870? 
sc:
- Increase in capital
- Increase in bank leverage*
- Nothing
success: Great, your answer is correct!
failure: Try again.
#>
...
#< award "Capital ratio"
Very good! Capital ratio is a key variable, but let's see how it performs regarding its predictive ability for financial crisis risk.
#>
...
#< award "Dplyr Level 3"
Awesome! You have learned how to use the `mutate()` function. Congratulations!
#>
...
```{r "5_5"}
#< hint
cat("Insert the variables ltd and year in the mutate() function")
#>
#< fill_in
...
The main functions for creating bar plots are `geom_bar()` and `geom_col()`. 

```{r "5_6",optional=TRUE, eval=FALSE}
geom_bar(
  mapping = NULL,
  data = NULL,
  stat = "count",
...
```{r "5_7",optional=TRUE, eval=FALSE}
geom_col(
  mapping = NULL,
  data = NULL,
  position = "stack",
...
stat: counts the number of observations at each x

position: "stack" (different groups atop each other) or "dodge" (different groups along the x-axis) 

fill: colour

The main difference of `geom_bar` to `geom_col()` is that `geom_bar()` uses `stat_count()` by default such that we don't have to define the y-values whereas `geom_col()` uses `stat_identity()` by default that leaves the data unchanged. Thus we need to hand over the y-values to the mapping.

Click on the following link to learn more about `geom_bar()` and `geom_col()`:
...
# LtD ratio in % from 2006 and 2015 grouped by iso
ggplot(ltd_data, aes(x = iso, y = ltd, fill = year)) +
  geom_col(position = "dodge", width = 0.6) + 
  labs(title = "LtD ratio in %, by country in 2006 and 2015", y = "LtD ratio in %")
```
...
# mean of LtD ratios in the sample countries between 1870 and 2015
summarise_ltd <- bankdata %>%
  group_by(year) %>%
  summarise(avgltd = mean(ltd, na.rm = TRUE)*100)

# Show first five rows of summarise_ltd
head(summarise_ltd, 5)
```
...
# Line and scatter plot of capital ratio averages by year for 17 countries
ggplot(summarise_ltd, aes(x = year, y = avgltd)) + 
...
As our dataset contains data up to 2015, we cannot capture the impact of the liquidity requirement on the LtD ratio. At this point I stumbled across the following statistics released by the *European Central Bank* in 2021. The statistic displays the aggregated LCR-ratio over all euro area countries (blue line).

<img src="Data/LCR.png" style="width: 70%; height: 70%; border:0.2px solid grey">

*Figure 15: "Liquidity coverage ratio and its components by reference period"; Source: European Central Bank, 2022*

The statistic shows that the LCR (blue line) increased from 138 % to 173 % from 2016 to 2021. After 2021, there is a slight decrease from 173 % to 164 %, which could be explained by the Corona pandemic and the difficult financial situation due to short-time work.
#>


#< award "Loan-to-deposits ratio"
Great! You have learned another important key variable which gained its importance in the recent years.
#>
...
# Aggregated data set by year
capitalstructure <- bankdata %>%
  group_by(year) %>%
  summarise(avglev = mean(lev, na.rm = TRUE)*100, 
            avgcor = mean(coreliabilities, na.rm = TRUE)*100) %>%
  mutate(max = 100)

# Show the first five lines of capitalstructure
head(capitalstructure, 5)
```
...
```{r "6_3",fig.width=9}
#< hint
cat("Capitalstructure, avgcor, avglev are the elements you need to enter.")
#>
#< fill_in
...
# 100% stacked area plot of capital structure
ggplot(data = capitalstructure) +
  geom_area(aes(x = year, y = max, fill = "Non-core"), alpha = 0.8) +
  geom_area(aes(x = year, y = avgcor, fill = "Deposits"), alpha = 0.8) +
  geom_area(aes(x = year, y = avglev, fill = "Capital"), alpha = 0.8) +
  geom_vline(aes(xintercept = 1945, color = "Maximum deposit ratio"), linetype = "dashed", linewidth = 0.8) +
  geom_vline(xintercept = 1983, colour = "purple", linetype = "dashed", linewidth = 0.8) +
  labs(y = "Share of total funding (%)", title = "Composition of liabilities, averages by year for 17 countries, full sample", fill = "Types of liabilities", colour = "") +
  scale_x_continuous(limits = c(1870, 2015), breaks = seq(1870, 2015, 20)) +
...
#< award "ggplot2 Level 2"
Great! You are able to plot an 100% stacked area plot which is not an easy task,
#>
...
#< award "Bank capital structure"
Great! "It doesn't matter whether a company is big or small. Capital structure matters. It always has and always will." *(Mike Milken)* 
#>
...
# Extract data of crisis years and and convert year into factor
crisisdata <- bankdata %>%
  filter(crisisJST == 1) %>%
...
# Extract data of crisis years and and convert year into factor
ggplot(crisisdata, aes(x = year, fill = country)) +
  geom_bar(width = 0.6, position = "stack") +
  theme(legend.position = "left", 
        axis.text = element_text(face="bold")) + 
  labs(x = "year", y = "Dimension", title="Crisis year and its dimension") +
...
# Line plot of key variables centered on the crisis year
ggplot(data = crisisyeardata) +
  geom_ribbon(aes(x = crisisyear, ymin = quantile25, ymax = quantile75, fill = "25th/75th percentile"), alpha = 0.5) +
...
bin_dat <- data.frame(
  dlev5 = bankdata$dlev5,
  bins = ntile(bankdata$dlev5, 5),
  crisisJST = bankdata$crisisJST
  ) %>%
  drop_na() 
...
#< award "ggplot2 Level 3"
Congratulations! You know how to use the `geom_boxplot()` command in order to create box plots.  
#>
...
# Calculate financial crisis frequency in year t
bins_freq <- bin_dat %>%
  # Group by bins and crisis
  group_by(bins, crisisJST) %>%
  summarise(n = n()) %>%
  mutate(freq_crisis = 100*n/sum(n)) %>%
  # Filter by crisisyear in order to get only the financial crisis frequency in year t
  filter(crisisJST == 1)
...
# bar plot
ggplot(data=bins_freq, aes(bins, freq_crisis)) +
  geom_col(fill = "darkgreen") +
  labs(x = "Quintiles of capital ratio changes between year t−6 and t−1", 
       y = "Financial crisis frequency in year t (%)", 
       title = "Capital ratio changes and crisis frequency") +
...
#< quiz "Capital ratio changes and crisis frequency"
question: What connection can you observe between financial crisis frequency and the capital ratio? 
sc:
- financial crisis frequency positively related to capital ratio changes
- financial crisis frequency negatively related to capital ratio changes
- financial crisis frequency unrelated to capital ratio changes*
success: Great, your answer is correct!
failure: Try again. 
#>
...
#< quiz "Distribution of a binary variable"
question: In linear probability models, the binary dependent variable can be seen as a random variable of a known distribution. Which distribution could it be?
sc:
...
success: Great, your answer is correct!
failure: Try again. Remember that $S_{it}$ is a binary variable.
#>
$$ $$
...
Whenever we use linear regression, we should critically check whether the corresponding assumptions are plausible for our data.  
...
$y$ is related to $x$ and $\epsilon$ as :
...
We have a random sample $\{ (x_i, y_i): i=1,2,...,n \}$.


**SLR.3: Sample Variation in the Explanatory Variable**

$\{ x_i: i=1,2,...,n \}$ are not all the same value.
...
#< quiz "Violation of linear regression model assumption"
question: Which of the assumptions is violated by the LPM model? Type the number of the corresponding assumption 1-5. 
answer: 5
#>
$$$$
...
#< info "lm()"
For linear regression, we use the `lm()` function. The name **lm** stands for linear model. 

This command uses the following basic syntax:
```{r "10_3",optional=TRUE, eval=FALSE}
lm(formula, data, ...)
```
...
- formula: 
the formula for the linear model, e.g if y is the dependent and x the independent variable, then our formula would look like as follows:

```{r "10_4",optional=TRUE, eval=FALSE}
y ~ x
```
...
If you want to know more about the `lm()` function, click [here](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/lm). 
...
# estimate a linar probability model
reg_lpm <- lm(crisisJST ~ lev_1, data = bankdata)

# Extract the coefficients to use the intercept and slope for the regression line
coeff <- coefficients(reg_lpm)
coeff
```

#< award "Regression Level 1"
You performed the first linear regression with the `lm()` function. Congratulations! 
#>
...
# Create scatterplot and regression line
ggplot(bankdata, aes(lev_1, crisisJST)) +
  geom_point() + 
  geom_hline(yintercept = c(0,1), linetype = "dashed", color = "blue") +
  geom_abline(aes(intercept = coeff[1], slope = coeff[2], colour = "Regressionline")) +
  scale_x_continuous(limits = c(-0.5, 6)) +
...
#< award "ggplot2 Level 4"
Congratulations! You know how to use the `geom_abline()` and `geom_hline()` command.  
#>
...
**Generalized linear models**, also called as **GLM**, are an important class of *nonlinear* models introduced by *John Nelder and Robert Wedderburn (1972)*, which is a generalization of the linear regression model. While in linear models the errors are assumed to be normally distributed, in GLM we can choose a distribution from the set of the exponential family. The set of exponential family contains for instance the Normal, Binomial, Poisson, Gamma, and inverse Gaussian distributions. Moreover, the variances no longer need to be *homogeneous* *(Hill, 2018)*.

The GLM consists of the following two elements:

a) A linear predictor, which is a linear combination of $\beta$ and the explanatory variable $X_{it}$: 
$$S_{it} = \alpha_{i}+\beta X_{it} + \epsilon_{it}$$

b) A link function g, which "links" the linear predictor with the expected value of the dependent variable:

$$\mathbb{E}[S_{it}|  \alpha_{i}, X_{it}] = g^{-1}(S_{it}) = g^{-1}(\alpha_{i}+\beta X_{it})$$

where by assumption $\mathbb{E}[\epsilon_{it}]=0$ and $g(\cdot)$ is invertible.
#>
...
Clustered standard errors occur when some observations in a data set are dependent on each other. This correlation occurs when individual characteristics within clusters are identical or similar. Since our data set is a panel data, and since panel data sets generally have clustering, observations within clusters could be correlated. 

Cluster-robust standard errors account for correlation or heteroscedasticity within the cluster and only influences the standard error and leave the estimates of our regression unchanged. If we ignore the clustering it will lead to underestimated standard errors *(Wooldridge, 2018)*.
#>
...
#< quiz "Probit regression"
question: How do we call the following expression $\Phi ^{-1}(P[S_{it} = 1 | \alpha_{i}, X_{it}])$? 
sc:
- p-score
- z-score*
- q-score
...
#< quiz "Mapping the first six lines of a data"
question: Now we will take a look at the data. Do you remember what function we had used to map the first six lines?
sc:
- str()
- first()
- head()*
...
The `mfx` R package provides functions that estimate a number of known generalised linear models and output marginal effects.

What are the advantages of the `mfx` package?

1. the `mfx` package provides the ability to estimate marginal effects.

2. `mfx` can estimate cluster-robust standard errors.

The marginal effects of the probit regression are estimated by the `probitmfx()` function:

```{r "12_5",echo=TRUE, eval=FALSE}
probitmfx(formula, data, 
          clustervar1 = NULL, 
          clustervar2 = NULL, 
          atmean = TRUE)
```
...
-`clustervar1`/`clustervar2`: name of first/second cluster for one or two-way clustered standard errors

-`atmean`: indicates whether marginal effects are calculated as partial effects for the average observation (`atmean = TRUE`) or as average partial effects (`atmean = FALSE`)

In case `atmean = TRUE`, the marginal effects are estimated by calculating the partial effects for the average observation *(Greene, 2008)*:

$$\frac{ \partial Pr[S = 1 | \alpha, X_{it}]}{ \partial X_{it,k}} = \phi (\alpha_{i}+\beta \bar{X}) * \beta_k$$

In case `atmean = FALSE`, the marginal effects are estimated by calculating the average partial effects *(Wooldridge, 2002)*:

$$\frac{ \partial Pr[S = 1 | \alpha, X_{it}]}{ \partial X_{it,k}} = \frac{\sum_{t=1}^T \sum_{i=1}^N \phi (\alpha_{i}+\beta X_{it})}{NT} * \beta_k$$

where $N$ is the number of total countries and $T$ the number of total years. According to *Greene (2008)* the difference of the different marginal effects is very small. 

You get more detailed information at https://cran.r-project.org/web/packages/mfx/vignettes/mfxarticle.pdf.
#>
...
# Probit regression 
reg1 <- probitmfx(crisisJST ~ dloansgdp5 + lev_1 + ccode,  data = reg_data, clustervar1 = "ccode")
```

#< award "Regression Level 2"
You performed your first probit model with the `probitmfx()` function. Congratulations! 
#>
...
#< quiz "Regression Question 1"
question: What kind of relationship does the probit regression capture between $\Delta_5 Loans/GDP$ and the likelihood of a crisis? 
sc:
- Credit variable is significantly negatively related to a higher probability of a crisis.
- Credit variable is insignificantly positively related to a higher probability of a crisis.
- Credit variable is significantly positively related to a higher probability of a crisis.*
success: Great, your answer is correct!
failure: Try again.
#>

#< quiz "Regression Question 2"
question: How would you describe the estimate of $\Delta_5 Loans/GDP$ in percentage? 
sc:
- 0.82 means that 2 percentage points increase in the credit variable would increase significantly the likelihood of a crisis by 0.82 percentage points.
- 0.82 means that 2 percentage points increase in the credit variable would decrease significantly the likelihood of a crisis by 1.64 percentage points.
- 0.82 means that 2 percentage points increase in the credit variable would increase significantly the likelihood of a crisis by 1.64 percentage points.*
success: Great, your answer is correct!
failure: Try again.
#>

#< quiz "Regression Question 3"
question: What kind of relationship does the regression capture between the one period lagged capital ratio and the crisis risk? 
sc:
- Higher capital is associated with lower crisis risk.
- Higher capital is associated with higher crisis risk.*
success: Great, your answer is correct!
failure: Try again.
#>
...
#< info "List Object"
Lists are a collection of different R objects, which can be structured in different ways. Lists can contain vectors, data sets, functions and lists. 

Lists are created quite simply with the `list()` function. In the following we build a list with inputs of different dimensions and variable types.

```{r "12_9",optional=TRUE, eval=FALSE}
data <- list(Name = "Max","Sarah",
             DateofBirth = as.Date(c("1997-11-30", "1993-11-12")),
             Salary = c(3000, 3500))
```

A list, named as `data`, has been created, which contains three elements with different types. The elements are named as `Name`, `DateofBirth` and `Salary`. We can now easily address these elements by their name `data$Name` or by their position `data[[1]]`. 

Click [here](https://www.tutorialspoint.com/r/r_lists.htm) to learn more about lists.
#>
...
probit_models <- list(
  # Full-data Capital ratio
  "(1) full" = probitmfx(crisisJST ~ dloansgdp5 + lev_1 + ccode,  data = reg_data, clustervar1 = "ccode"),
  # Full-data loan-to-deposits ratio
  "(2) full" = probitmfx(crisisJST ~ dloansgdp5 + ltd_1 + ccode, data = reg_data, clustervar1 = "ccode"),
  # Full-data Noncore ratio
  "(3) full" = probitmfx(crisisJST ~ dloansgdp5 + noncore_1 + ccode, data = reg_data, clustervar1 = "ccode"),
  # Post-data Capital ratio
  "(4) post" = probitmfx(crisisJST ~ dloansgdp5 + lev_1 + ccode,  data = reg_post, clustervar1 = "ccode"),
  # Post-data loan-to-deposits ratio
  "(5) post" = probitmfx(crisisJST ~ dloansgdp5 + ltd_1 + ccode, data = reg_post, clustervar1 = "ccode"),
  # Post-data Noncore ratio
  "(6) post" = probitmfx(crisisJST ~ dloansgdp5 + noncore_1 + ccode, data = reg_post, clustervar1 = "ccode")
)
...
#< award "htmlreg()"
Congratulations! You know how to use the `htmlreg()` command.  
#>

Now it is time for some questions regarding the regression table:

#< quiz "Regression Question 4"
question: How would you describe the estimate of the LtD ratio in percentage? 
sc:
- 0.04** means that 2 percentage points increase in LtD ratio would increase the likelihood of a crisis significantly by 0.08 percentage points.*
- 0.04** means that 2 percentage points increase in LtD ratio would decrease the likelihood of a crisis significantly by 0.08 percentage points.
- 0.04** means that 2 percentage points increase in LtD ratio would increase the likelihood of a crisis significantly by 0.1 percentage points.
success: Great, your answer is correct!
failure: Try again.
#>
...
#< quiz "Regression Question 5"
question: How would you interpret the coefficient of the non-core ratio in the post-war model? 
sc:
- Non-core ratio enters significantly with the expected sign.*
- Non-core ratio enters insignificantly with the expected sign.
- Non-core ratio enters significantly with an unexpected sign.
success: Great, your answer is correct!
failure: Try again.
#>
...
ROC-curves are created with the `roc()` function from the `pROC` package. Its return is a list which can be used for further analyses, like calculating the AUC or plotting the ROC-curve. 

```{r "13_1",echo=TRUE, eval=FALSE}
# ROC-curve
roc(response, predictor, auc = TRUE)
```

The predictor need to be determined in advance with the function `predict(model, data)`. 

For more information click [here](https://www.rdocumentation.org/packages/pROC/versions/1.18.0/topics/roc). 
...
This cheat sheet includes all models which are considered in this chapter. The list will be updated after each definition of a model in order to have a good overview.

**Model 0:** Country fixed effects (full)

**Model 1:** Country fixed effects (post)
...
In R generalised linear models can be fitted with the `glm()` command from the `stats` package. The function is structured as follows:

```{r "13_3",echo=TRUE, eval=FALSE}
glm(formula, data, family, ...)
```

where the argument `family` specifies which link function is to be used: 
- Per default: `family = gaussian`
- If the dependent variable y is a binary variable: `family = binomial()`
  - Logit regression: `family = binomial(link = logit)` (per default)
  - Probit regression: `family = binomial(link = probit)` 

For more information about the `glm()` function, click [here](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/glm).
#>
...
# Sample solution
# Benchmark null model - full and post
probmodel_0 <- glm(data = reg_full, crisisJST ~ factor(ccode), family=binomial(probit))
probmodel_1 <- glm(data = reg_post, crisisJST ~ factor(ccode), family=binomial(probit))
```

#< award "Regression Level 3"
Great! You performed the first generalized linear model with the `glm()` function. 
#>
...
# Calculate predicted values
pred_0 <- predict(probmodel_0, reg_full)
pred_1 <- predict(probmodel_1, reg_post)
```
...
# Call roc() and insert crisisJST and prediction values
# Save roc-objects in a list
model0 <- roc(reg_full$crisisJST, pred_0)
model1 <- roc(reg_post$crisisJST, pred_1)
```
...
The `annotate()` function from the `ggplot2` package allows us to add geoms like text labels or rectangles to a plot.

The function is used as follows:
```{r "13_8",echo=TRUE, eval=FALSE}
annotate(
  geom,
  x = NULL,
  y = NULL)
...
where x and y are used to define the position of the object.

Click [here](https://ggplot2.tidyverse.org/reference/annotate.html) for more.
#>
...
# Sample solution
# ROC curves
ggroc(list(model0 = model0, model1 = model1), legacy.axes = "TRUE", size = 1) +
  geom_abline(linetype = "dashed", color = "blue") +
  annotate("text", x = 0.5, y = 0.45, label = "no predictive value", angle = 62, size = 3) +
  labs(title = "ROC curve", color = "Model") +
  theme_bw() + 
...
#< award "ROC and AUC"
Great! You calculated the AUC values and plotted the ROC curve with the `roc()` and `ggroc()` functions. 
#>
...
**Model 2:** Loans/GDP (full)

**Model 3:** Loans/GDP (post)
#>
...
rates_0 <- data.frame(fpr = 1- model0$specificities, tpr = model0$sensitivities)
rates_1 <- data.frame(fpr = 1- model1$specificities, tpr = model1$sensitivities)
rates_2 <- data.frame(fpr = 1- model2$specificities, tpr = model2$sensitivities)
rates_3 <- data.frame(fpr = 1- model3$specificities, tpr = model3$sensitivities)
```
...
plot_0_2 <- ggplot() +
    geom_line(data = rates_0, aes(fpr, tpr, color = "Model 0"), colour = "red", linewidth = 1) + 
    geom_line(data = rates_2, aes(fpr, tpr, color = "Model 2"), colour = "blue", linewidth = 0.7) + 
    labs(title = paste0("ROC-curve - Full sample"), x = "FPR (1-Specificity)", y = "TPR (Sensitivity)") +
    geom_abline(linetype = "dashed")
...
plot_1_3 <- ggplot() +
    geom_line(data = rates_1, aes(fpr, tpr, colour = "Model 1"), colour = "red", linewidth = 1) + 
    geom_line(data = rates_3, aes(fpr, tpr, colour = "Model 3"), colour = "blue", linewidth = 0.7) + 
    labs(title = paste0("ROC-curve - Post sample"), x = "FPR (1-Specificity)", y = "TPR (Sensitivity)") +
    geom_abline(linetype = "dashed") 
...
**Model 4:** Loans/GDP, capital ratio (full)

**Model 5:** Loans/GDP, LtD ratio (full)

**Model 6:** Loans/GDP, non-core ratio (full)

**Model 7:** Loans/GDP, capital ratio (post)

**Model 8:** Loans/GDP, LtD ratio (post)

**Model 9:** Loans/GDP, non-core ratio (post)
...
prodmodel_5 <- glm(data = reg_full, crisisJST ~ dloansgdp5 + ltd_1 + factor(ccode), family=binomial(probit))
prodmodel_6 <- glm(data = reg_full, crisisJST ~ dloansgdp5 + noncore_1 + factor(ccode), family=binomial(probit))
prodmodel_7 <- glm(data = reg_post, crisisJST ~ dloansgdp5 + lev_1 + factor(ccode), family=binomial(probit))
prodmodel_8 <- glm(data = reg_post, crisisJST ~ dloansgdp5 + ltd_1 + factor(ccode), family=binomial(probit))
prodmodel_9 <- glm(data = reg_post, crisisJST ~ dloansgdp5 + noncore_1 + factor(ccode), family=binomial(probit))
```
...
pred_5 <- predict(prodmodel_5, reg_full)
pred_6 <- predict(prodmodel_6, reg_full)
pred_7 <- predict(prodmodel_7, reg_post)
pred_8 <- predict(prodmodel_8, reg_post)
pred_9 <- predict(prodmodel_9, reg_post)
```
...
# Sample solution
# Calculate FPR and TPR of post-WW2 models (7)-(9)
rates_7 <- data.frame(fpr = 1 - roc(reg_post$crisisJST, pred_7)$specificities, tpr = roc(reg_post$crisisJST, pred_7)$sensitivities)
rates_8 <- data.frame(fpr = 1 - roc(reg_post$crisisJST, pred_8)$specificities, tpr = roc(reg_post$crisisJST, pred_8)$sensitivities)
rates_9 <- data.frame(fpr = 1 - roc(reg_post$crisisJST, pred_9)$specificities, tpr = roc(reg_post$crisisJST, pred_9)$sensitivities)
```
...
# Place all plots in a rectangular grid
grid.arrange(ggplot_list[[4]], ggplot_list[[5]], ggplot_list[[6]], ggplot_list[[7]], ggplot_list[[8]], ggplot_list[[9]], nrow = 3, ncol = 2)
...
#< award "for-loops"
Congratulations! You know how to use for loops to execute a function iteratively. 
#>
...
- **dlcpi5**: 5-year average changes of annual inflation growth
- **dlrgdp5**: 5-year average changes of real GDP index growth
- **stir5**: 5-year average changes of short-term interest rate
- **vol_dlrgdp_5**: volatility of real GDP index growth over the preceding 5-year period
- **vol_dlcpi_5**: volatility of annual inflation growth in % over the preceding 5-year period
- **vol_stir_5**: volatility of short-term interest rate growth over the preceding 5-year period
- **vol_dloansgdp_5**: volatility of credit-to-GDP growth over the preceding 5-year period

--> asset price controls:

- **bank_return_1**: 1-year lagged bank index excess returns
- **bank_return_2**: 2-year lagged bank index excess returns
- **bank_return_3**: 3-year lagged bank index excess returns
- **dlrhouses5**: 5-year average changes of real HP index 
- **vol_dlrhouses_5**: volatilities of real HP index over the preceding 5-year period
...
# Load data
data_addcont <- readRDS("Regressiondata_addcont.rds") 

# macroeconomic controls
model_addcont1 <- probitmfx(crisisJST ~ lev_1 + dloansgdp5 + vol_dlrgdp5 + vol_dlcpi5 +  vol_stir5 + dlcpi5 + dlrgdp5 + stir5 + vol_dloansgdp5 + ccode,  data = data_addcont, clustervar1 = "iso")

# macroeconomic and asset price controls
model_addcont2 <- probitmfx(crisisJST ~ lev_1 + dloansgdp5 + vol_dlrgdp5 + vol_dlcpi5 +  vol_stir5 + dlcpi5 + dlrgdp5 + stir5 + vol_dloansgdp5 + bank_return_3 + bank_return_2 + bank_return_1 + dlrhouses5 + vol_dlrhouses5 + ccode,  data = data_addcont, clustervar1 = "iso")
...
#Load data
data_addcont_post <- readRDS("Regressiondata_addcont_post.rds") 

# fit model which includes macroeconomic controls for post-war data
model_addcont1_post <- probitmfx(crisisJST ~ lev_1 + dloansgdp5 + vol_dlrgdp5 + vol_dlcpi5 +  vol_stir5 + dlcpi5 + dlrgdp5 + stir5 + vol_dloansgdp5 + ccode,  data = data_addcont_post, clustervar1 = "iso")

# fit model which includes macroeconomic and asset price controls for post-war data
model_addcont2_post <- probitmfx(crisisJST ~ lev_1 + dloansgdp5 + vol_dlrgdp5 + vol_dlcpi5 +  vol_stir5 + dlcpi5 + dlrgdp5 + stir5 + vol_dloansgdp5 + bank_return_3 + bank_return_2 + bank_return_1 + dlrhouses5 + vol_dlrhouses5 + ccode,  data = data_addcont_post, clustervar1 = "iso")
```
...
#< quiz "Controlling for asset risk"
question: What do you think? Does the above result differ from the results of the baseline model?  
sc:
- Yes.
- No.*
success: Great, your answer is correct!
...
#< quiz "Presence of deposit insurance - Question 1"
question: Do we get the same results for capital ratios as in the baseline model, if we consider observations with deposit insurance? 
sc:
- Yes. An increase in capital ratios means a significant increase in the crisis risk.
- No. An increase in capital ratios means an insignificant increase in the crisis risk.
- No. An increase in capital ratios means a significant decrease in the crisis risk.
- No. An increase in capital ratios means an insignificant decrease in the crisis risk.*
success: Great, your answer is correct!
failure: Try again. Take a look at the second column.
#>
...
#Sample solution
# Evolution of the log real GDP index
ggplot(bankdata, aes(year, lrgdp, color = country)) +
  geom_line() +
  ylab("Real GDP per capita (%)") +
  ggtitle("Real GDP per capita by country, full sample") +
  scale_x_continuous(limits = c(1870, 2015)) +
...
The economic development of a country follows a wave-like pattern. The business cycle can be divided into four phases: 

<img src="Data/Businesscycle.png" style="width: 60%; height: 60%"> 

*Figure 37: "Businesscycle"; Source: own illustration*


Phase 1: Boom
- demand for goods and services marks a peak
- production capacities are fully utilised
- increase in wages
...
- increase in short-time work and unemployment rate
- decrease in prices and interest rates
...
- increase in employment rate and wages due to increased production
...
# Real GDP growth distribution around financial crises
ggplot(data = crisisyeardata) +
  geom_ribbon(aes(x = year, ymin = quantile25, ymax = quantile75, fill = "25% - 75%"), alpha = 0.7) +
  geom_line(aes(x = year, y = median, color = "Median"), linewidth = 1) +
  geom_vline(xintercept = 0, linewidth = 0.5, linetype = "dashed", colour = "#CC6666") +
  geom_hline(yintercept = 0, linewidth = 0.3, linetype = "dashed")  +
  geom_text(aes(-1.9, -0.3, label = "Start of a systemic crisis"), size = 4, colour = "#CC6666") +
  labs(y = "Real GDP growth in %", title = "Real GDP growth distribution around financial crises") +
...
#< award "ggplot2 Expert"
Congratulations! You know how to use the most important functions from the `ggplot2` package.  
#>
...
#< quiz "Question 1"
question: How many normal recessions did Italy experienced during the observation period?
sc:
- 10*
- 12
- 14
...
#< quiz "Question 2"
question: How many financial recessions did Switzerland face during the observation period?
sc:
- 5*
- 17
- 22
...
#< quiz "Question 3"
question: How many financial recessions can we observe in total?
sc:
- 75*
- 73
- 76
...
# Calculate mean of capital ratio during financial recession period
mean_lev1 <- mean(filter(LP_cont, pk_fin == 1)$lev_1, na.rm = TRUE)

# add pk_finlo and pk_finhi to LP_cont depending on level of 1-year lagged capital ratio
LP_cont <- LP_cont %>%
    mutate(pk_finlo =  ifelse(is.na(lev_1), NA, ifelse(lev_1 <= mean_lev1, pk_fin, 0)), 
           pk_finhi = ifelse(is.na(lev_1), NA, ifelse(lev_1 > mean_lev1, pk_fin, 0))
           ) 
...
# Load library
ggplot(boxplot_data, aes(x = boxplot_group, y = lrgdp, col = boxplot_group)) +
  geom_boxplot() +
  labs(x = "Type of recession", y = "real GDP per capita", title = "Normal recession vs financial recession (high/low)", colour = "") + 
...
The `felm()` function from the package `lfe()` is used to fit linear models with multiple group fixed effects, instrumental variables or multi-way clustered errors.

```{r "17_2",optional=TRUE, eval=FALSE}
felm(
  formula,
  data,
  ...
...
The right side of the formula consists of four parts:

```{r "17_3",optional=TRUE, eval=FALSE}
y ~ a | b | c | d
```

where 

- a: ordinary covariates
- b: fixed effects
- c: instrumental variables
- d: multi-way clustered errors

If you want to learn more about the `lfe()` package and its functions, just read through the following article:
...
# h = 1
reg1<- felm(gf(lrgdp_1 ~ {exp_variables}|0|0|iso), data = LP_cont)

# h = 2
reg2 <- felm(gf(lrgdp_2 ~ {exp_variables}|0|0|iso), data = LP_cont)

# h = 3
reg3 <- felm(gf(lrgdp_3 ~ {exp_variables}|0|0|iso), data = LP_cont)

# h = 4
reg4 <- felm(gf(lrgdp_4 ~ {exp_variables}|0|0|iso), data = LP_cont)

# h = 5
reg5 <- felm(gf(lrgdp_5 ~ {exp_variables}|0|0|iso), data = LP_cont)

# h = 99
regsum <-felm(gf(lrgdp_99 ~ {exp_variables}|0|0|iso), data = LP_cont)

# Save all models in a list
modelsev <- list(reg1, reg2, reg3, reg4, reg5, regsum)
```

#< award "Regression Level 4"
Excellent! You are able to perform regressions with the `felm()` function from the `lfe` package.
#>
...
#< info "stargazer()"
In order to present the results of the regression clearly, we use regression tables. We create this using the `stargazer()` function of the *stargazer* package. `stargazer()` is usually used to present the output of the `lm()` function in a LaTeX, HTML or ASCII format. The function is structured as follows:

```{r "17_6",optional=TRUE, eval=FALSE}
stargazer( model,
           type = "latex", 
           title = "",
           dep.var.labels = NULL,
...
- model: model objects or model list

- type: type of output ("html", "latex" or "text")

- title: title as character vector

- dep.var.labels: labels for dependent variables as character vector

- keep: a vector of explanatory variables that should be kept

- omit.stat: a vector of model statistics that should be omitted

- model.numbers: indicates whether the model numbers should be included

For more information, click [here](https://cran.r-project.org/web/packages/stargazer/stargazer.pdf).
#>
...
#< award "stargazer()"
Well done! You are able to present regression results using the `stargazer()` function.
#>
...
# Evolution of lriy and dlriy over 1870-2015
ggplot(data = LP_cont) +
  geom_line(aes(x = year, y = lriy, color = "lriy", group = iso)) +
  geom_line(aes(x = year, y = dlriy/100, color = "dlriy", group = iso)) +
  labs(y = "Control variable", colour = "Control variable") +
  scale_x_continuous(breaks = seq(1870, 2015, 20))
...
- c_dlrgdp: centered first difference of log real GDP
- c_dlcpi:  centered first difference of log consumer Price Index
- c_dlriy:  centered first difference of log real investment-to-GDP ratio
- c_stir:   centered short-term interest rate
- c_ltrate: centered long-term interest rate
- c_cay:    centered current account over GDP ratio
- c_ldlrgdp:  centered one year lagged first difference of log real GDP
- c_ldlcpi:   centered one year lagged first difference of log consumer Price Index
- c_ldlriy:   centered one year lagged first difference of log real investment-to-GDP ratio
- c_lstir:    centered one year lagged of short-term interest rate
- c_lltrate:    centered one year lagged of long-term interest rate
- c_lcay:   centered one year lagged of current account over GDP ratio
#>
...
The apply-family contains seven functions, all ending in apply. Each of the functions has at least two arguments: an *object* and a *function*. We will look at the three most commonly used functions:
`apply()`, `sapply()` and `lapply()`

In the following image, I have shown the most important task of each apply function:
**split - apply - combine**

<img src="Data/Apply.png" style="width: 60%; height: 60%">

*Figure 44: "apply()-function"; Source: own illustration*

**apply()**:

You can use the `apply()` function to apply any (suitable) function to rows and columns of a matrix or data set.`MARGIN` specifies the index to which the function should be applied: `MARGIN = 1` means row by row,  `MARGIN = 2` means column by column and `MARGIN = c(1, 2)` means by rows and column.  

```{r "17_11",optional=TRUE, eval=FALSE}
apply(X, MARGIN, FUN, …)
```

**lapply()**:

This function takes *lists* or *vectors* as inputs and returns a *list*. The function is applied to each element of the vector.

```{r "17_12",optional=TRUE, eval=FALSE}
lapply(X, FUN, ...)
```

**sapply()**:

This function takes *lists* or *vectors* as inputs and returns, if possible, a *vector* or *matrix*. Otherwise it will return a *list*. The function is applied to each element of the vector.

```{r "17_13",optional=TRUE, eval=FALSE}
sapply(X, FUN, ...)
```

Have a look at https://ademos.people.uic.edu/Chapter4.html for more information about the `apply()` functions.
#>
...
# Function to extract the coefficients of Recession, pk_finhi and pk_finlo
coefficients <- function(i){
  coefficients <- c(coef(model_sev[[i]])[1],
                    coef(model_sev[[i]])[1] + coef(model_sev[[i]])[2],
                    coef(model_sev[[i]])[1] + coef(model_sev[[i]])[3])
}
...
NFdat <- sapply(seq(1,5), coefficients)
...
# Sample solution
ggplot(data = NF_plot) +
  geom_line(aes(x = c(0:5), y = pk_finhi, color = "Financial, high capital ratio")) +
  geom_line(aes(x = c(0:5), y = pk_finlo, color = "Financial, low capital ratio")) +
  geom_line(aes(x = c(0:5), y = Recession, color = "Normal recession")) +
  geom_hline(yintercept = 0) +
  labs(x = "Year", y = "Percent (100x log)", title = "Normal vs financial recessions, real GDP per capita by capital ratio", colour = "") +
  theme(legend.position="bottom") +
  theme_bw()
...
# Linear model with fixed effect and country-clustered errors
reg_pre2006_1<- felm(gf(lrgdp_1 ~ {exp_variables}|0|0|iso), data = pre2006)
reg_pre2006_2 <- felm(gf(lrgdp_2 ~ {exp_variables}|0|0|iso), data = pre2006)
reg_pre2006_3 <- felm(gf(lrgdp_3 ~ {exp_variables}|0|0|iso), data = pre2006)
reg_pre2006_4 <- felm(gf(lrgdp_4 ~ {exp_variables}|0|0|iso), data = pre2006)
reg_pre2006_5 <- felm(gf(lrgdp_5 ~ {exp_variables}|0|0|iso), data = pre2006)
reg_pre2006_sum <-felm(gf(lrgdp_99 ~ {exp_variables}|0|0|iso), data = pre2006)
